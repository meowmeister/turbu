...
interface
uses
   windows, contnrs, syncObjs, //windows libs
   commons, chipset_data, frames, //turbu libs
...
type
   ...
   public
      constructor Create(parent: TSpriteEngine; coords: TRect; main: TMenuEngine; owner: TMenuPage); reintroduce;
   ...
   protected
      ...
      FComponent: array of TSystemFrame;
      FBounds: TRect;

      procedure setVisible(const value: boolean); virtual;
      procedure registerComponent(which: TSystemFrame);
   public
      constructor Create(parent: TSpriteEngine; coords: TRect; main: TMenuEngine); virtual;
...
constructor TGameMenuBox.Create(parent: TSpriteEngine; coords: TRect; main: TMenuEngine; owner: TMenuPage);
begin
   FMenuEngine := main;
   FOwner := owner;
   name := self.ClassName;
...
constructor TMenuEngine.Create(parent: TSpriteEngine);
begin
   assert(parent is TGameMap);
   inherited Create;
   FParent := parent;
   FCursor := TGameMap(parent).cursor;
...
var dummy: TMenuPage;
begin
   dummy := TMenuPage(FStack.pop);
   if dummy = nil then
      self.leave(false)
   else begin
      FCurrentPage := dummy;
      dummy.setup(CURSOR_UNCHANGED);
   end;
end;
...
constructor TMenuPage.Create(parent: TSpriteEngine; coords: TRect;
  main: TMenuEngine);
begin
   inherited Create;
   FBounds := coords;
...
procedure TMenuPage.registerComponent(which: TSystemFrame);
begin
